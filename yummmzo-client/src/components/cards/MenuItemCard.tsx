...
import { ConflictCartDialogComponent } from "../modals/ConflictCartDialogComponent";

export function MenuItemCard({ item, index = 0, restaurantId }: { item: any; index?: number; restaurantId: number }) {
    // useCart
    const { addItem, updateItem, getItemQuantity, clearCart, conflictError, setConflictError } = useCart();
    const quantity = getItemQuantity(item.id);

    // State Variables
    const [pendingItem, setPendingItem] = useState<{ restaurantId: number, menuItemId: number, quantity: number } | null>(null);

    // Handler Functions
    const handleAddItem = () => {
        const payload = { restaurantId, menuItemId: item.id, quantity: 1 };
                                    ...
                                    (
                                        <div className="flex items-center gap-2">
                                            <Button
                                                ...
                                                onClick={() => updateItem({ cartItemId: item.id, quantity: quantity - 1 })}
                                            >
